...
<div class="main"><div class="global-width">
    ...
    <div class="action">
		...
		<tr  align="center"> 
			...
			<td>
				<a href="<%=path%>/email/recover/${mail.id}"><img src="<%=path %>/resources/images/save.gif" width="16" height="16" /></a>
				<a href="<%=path%>/email/remove/${mail.id}"><img src="<%=path %>/resources/images/delete.gif" width="16" height="16" /></a>
			</td>
		</tr>
 		</c:forEach>
        <tr>
        <td colspan="6" align="center">    
	 		...
	 		<c:if test="${page.currentPage > 1}">    
		        <a href="<%=path %>/email/trash?p=1">第一页</a>    
				<a href="<%=path %>/email/trash?p=${page.currentPage-1}">上一页</a>    
         	</c:if>
         	
         	<c:if test="${page.currentPage==page.pageSum }">下一页  最后一页</c:if>
         	<c:if test="${page.currentPage < page.pageSum}">    
				<a href="<%=path %>/email/trash?p=${page.currentPage+1}">下一页</a>    
				<a href="<%=path %>/email/trash?p=${page.pageSum}">最后一页</a>    
         	</c:if>        
     	</td>        
        </tr>
        </table>
